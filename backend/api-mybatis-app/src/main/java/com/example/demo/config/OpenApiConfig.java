...
public class OpenApiConfig {
    ...
    OpenAPI openApi() {
        return new OpenAPI().info(new Info()
                .title("api-mybatis-app")
                .description("MyBatis 조회 + 공통 응답 { status, data, error }. 맨 아래 Schemas 의 「Product (Request + Response)」에서 요청/응답 필드를 같이 본다.")
                .version("1.0.0"));
    }

    /** Swagger 하단 Schemas 에 Request/Response 를 한 모델로 묶는다. */
    @Bean
