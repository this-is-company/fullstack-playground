package com.example.enumapp.web.dto;

import com.example.enumapp.web.validation.LocalDateString;
import com.example.enumapp.web.validation.LocalDateTimeString;
import com.example.enumapp.web.validation.NoNullElements;
import com.example.enumapp.web.validation.NoSpecialChars;
import io.swagger.v3.oas.annotations.media.Schema;
import jakarta.validation.Valid;
import jakarta.validation.constraints.Min;
import jakarta.validation.constraints.NotBlank;
import jakarta.validation.constraints.NotEmpty;
import jakarta.validation.constraints.NotNull;
import jakarta.validation.constraints.Positive;

import java.util.List;

/**
 * GET 쿼리용 검증 DTO.
 * <ul>
 *   <li>ids — 리스트 null/empty + 요소 null 금지</li>
 *   <li>items — 중첩 객체 리스트, 내부 필드 null/blank/숫자 검증</li>
 *   <li>날짜 — 문자열로 받아 형식 검증 후 LocalDate/LocalDateTime 변환</li>
 *   <li>숫자 — Wrapper 로 null 유지, 값이 있으면 범위 검증</li>
 * </ul>
 *
 * 예: {@code GET ...?ids=1&ids=2&minQuantity=5&orderDate=2026-08-06
 * &items[0].sku=ABC&items[0].quantity=2}
 */
@Schema(description = "GET 쿼리 검증 데모 파라미터")
public class GetValidationQuery {

    @NotEmpty(message = "ids must not be null or empty")
    @NoNullElements
    @Schema(description = "주문 id 목록. 반복 파라미터: ids=1&ids=2", requiredMode = Schema.RequiredMode.REQUIRED)
    private List<Long> ids;

    @Valid
    @NotEmpty(message = "items must not be null or empty")
    @NoNullElements
    @Schema(description = "중첩 필터. items[0].sku=..&items[0].quantity=..", requiredMode = Schema.RequiredMode.REQUIRED)
    private List<ItemFilter> items;

    @Min(value = 0, message = "minQuantity must be >= 0")
    @Schema(description = "최소 수량. 생략(null) 가능, 있으면 0 이상", nullable = true, example = "5")
    private Integer minQuantity;

    @Positive(message = "page must be positive")
    @Schema(description = "페이지(선택). 있으면 양수", nullable = true, example = "1")
    private Integer page;

    @LocalDateString
    @Schema(description = "단일 일자 문자열 yyyy-MM-dd", example = "2026-08-06")
    private String orderDate;

    @LocalDateString
    @Schema(description = "시작일 yyyy-MM-dd", example = "2026-08-01")
    private String fromDate;

    @LocalDateString
    @Schema(description = "종료일 yyyy-MM-dd", example = "2026-08-03")
    private String toDate;

    @LocalDateTimeString
    @Schema(description = "시작일시 yyyy-MM-dd'T'HH:mm:ss", example = "2026-08-06T09:00:00")
    private String fromDateTime;

    @LocalDateTimeString
    @Schema(description = "종료일시 yyyy-MM-dd'T'HH:mm:ss", example = "2026-08-06T18:00:00")
    private String toDateTime;

    public List<Long> getIds() {
        return ids;
    }

    public void setIds(List<Long> ids) {
        this.ids = ids;
    }

    public List<ItemFilter> getItems() {
        return items;
    }

    public void setItems(List<ItemFilter> items) {
        this.items = items;
    }

    public Integer getMinQuantity() {
        return minQuantity;
    }

    public void setMinQuantity(Integer minQuantity) {
        this.minQuantity = minQuantity;
    }

    public Integer getPage() {
        return page;
    }

    public void setPage(Integer page) {
        this.page = page;
    }

    public String getOrderDate() {
        return orderDate;
    }

    public void setOrderDate(String orderDate) {
        this.orderDate = orderDate;
    }

    public String getFromDate() {
        return fromDate;
    }

    public void setFromDate(String fromDate) {
        this.fromDate = fromDate;
    }

    public String getToDate() {
        return toDate;
    }

    public void setToDate(String toDate) {
        this.toDate = toDate;
    }

    public String getFromDateTime() {
        return fromDateTime;
    }

    public void setFromDateTime(String fromDateTime) {
        this.fromDateTime = fromDateTime;
    }

    public String getToDateTime() {
        return toDateTime;
    }

    public void setToDateTime(String toDateTime) {
        this.toDateTime = toDateTime;
    }

    public static class ItemFilter {

        @NotBlank(message = "sku must not be blank")
        @NoSpecialChars
        @Schema(example = "SKU-001")
        private String sku;

        @NotNull(message = "quantity must not be null")
        @Positive(message = "quantity must be positive")
        @Schema(example = "2")
        private Integer quantity;

        public String getSku() {
            return sku;
        }

        public void setSku(String sku) {
            this.sku = sku;
        }

        public Integer getQuantity() {
            return quantity;
        }

        public void setQuantity(Integer quantity) {
            this.quantity = quantity;
        }
    }
}
